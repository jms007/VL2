 ...
 * @author jms
 */
/* This class (1) builds a Jtree representation of the chart,
 *            (2) puts the tree into a scroll pane, and
 *            (3) enables single-node selection,
 *        but does not
 *            (1) specify the location or the size of its display,
 *            (2) make the tree visible, or
 *            (3) enable tree selection events beyond this class.
 * 
 * Use class utilities.DisplayTree to position and size the frame 
 *     and display the tree.
 *
 * Classes that need to use the Chart Tree node selection functionality
 * must declare themselves to be listeners to node-selection events:
 *    (1) implements TreeSelectionListener
 *    (2) put tree in parameter list
 *    (3) tree.addSelectionListener(this);
 *    (4) add method valueChanged(TreeSelectionEvent e)
    ...
    implements MouseListener //,TreeSelectionListener
{
    ...
    ChartTree( XProperties props, Chart chart, LogFile logger )
...
//            new ComputeTotals( props, chart, begin, end, 0, "", logger );
    ...
    /* This method is not used in the current (1-6-2018) implementation.
    public ChartElement[] initFromXML(String chartFilename)
        throws ParserConfigurationException, SAXException, IOException
    {
        ...
        ParseDocument pd = new ParseDocument( chartFilename, logger );
        elementList = pd.getElementList();
        logger.log("initFromXML returning elementList of length "+ elementList.length);
        logger.setLogLevel(LogFile.NORMAL_LOG_LEVEL);
        return elementList;
    }
*/

    private JTree buildTree( ChartElement elementList[] )
        throws VLException
    {
        //logger.setLogLevel(LogFile.DEBUG_LOG_LEVEL);
        ...
        // The default is for button 1, double-click to expand/collapse the selected node, unless it is a leaf.
        // So we alter this behavior to require a triple-click on button 1 to do that, so we can use
        // the double-click for our messages (see mouseClicked() below)
        tree.setToggleClickCount(3);
        return tree;
    }

    private int build( ChartElement elementList[], int index, ChartTreeNode parent, int level )
    throws VLException
    {
        ...
        while( elementList[index].getLevel() == level )
        {
            ...
            if ( isElement( elementList[index], new String[] { "chart", "section", "group" } ) )
            {
                ...
                if ( index >= elementList.length ) return index;
            }
            else
            {   // Either an Account or a Total element
                ...
                {   // Bypass warnings
                    ...
                    /*****
                    {   // To include transactions in the tree:
                        // If this is an account element, use the node just created (accountNode) as parent
                        // and add the items in GLEntries from that account as new DefaultMutableTreeNode's
                        // formatted with simple String's
                        // This block (marked with / 5 stars: ***** ) can be removed if you do not want this feature
                        if ( isElement( elementList[index], new String[] { "account" } ) )
                        {
                            Account account = chart.getAccount( elementList[index].getAccountIndex() );
                            for (int i=0; i<account.glEntries.size(); ++i)
                            {
                                GLEntry glEntry = account.glEntries.elementAt( i );
                                String transString = glEntry.getJulianDate().toString( "mm-dd") + " " +
                                    Strings.formatPennies( glEntry.getLongAmount(), "," ) + " " +
                                    glEntry.getDescr();
                                // These nodes are not ChartTreeNode's ... maybe a problem somewhere ...
                                DefaultMutableTreeNode transNode = new DefaultMutableTreeNode( transString );
    ...
    private String formatNode( ChartElement element, String nodeDescrSpec )
    throws VLException
    {
        String nodeData = ""; 
        // There are no options for the text-only elements
        if ( element.getName().equals( "chart" )
          || element.getName().equals( "section" )
...
//    }
    ...
    public void mouseClicked( MouseEvent e)
    {
        ...
        {
            //Console.println( "dispatch button " + button + " " + clickCount + " click(s) on " + selectedNode.toString() );
            // Send message only if this is an account node
            //if ( Strings.regexMatch( "\\[\\d+\\].*", selectedNode.toString() ) ) // doesn't work for acct/subacct
...
//            {   // left mouse button (this will expand/collapse the selected node, unless it is a leaf)
//                Console.println( "dispatch button 1 doubleClick on " + selectedNode.toString() );
//                firePropertyChange( "1:2:" + selectedNode.toString() );
//                e.consume();
//            }
//            else if ( button == 2 )
//            {   // mouse "wheel" button
//                Console.println( "dispatch button 2  " + e.getClickCount() + " click on " + selectedNode.toString() );
//                firePropertyChange( "2:"+ e.getClickCount() + ":" + selectedNode.toString() );
//            }
//            else if ( button == 3 )
//            {   // right mouse button
//                Console.println( "dispatch button 3  " + e.getClickCount() + " click on " + selectedNode.toString() );
//                firePropertyChange( "3:1:" + selectedNode.toString() );
//            }
        }
    }

    // This is our homemade inter-class messaging service, piggybacking on PropertyChange Events
    ...
    public static boolean isElement( ChartElement element, String[] elementNames )
    {
        for (int i=0; i<elementNames.length; ++i)
            if ( element.name.equals( elementNames[i] ) ) return true;
        return false;
    }
    ...
    public static void main(String[] args)
    {
        try
        {
            Clip clip = new Clip( args, new String[]
                { "d=E:\\ACCOUNTING\\EXTANT\\GL17\\"
                , "p=E:\\ACCOUNTING\\EXTANT\\EXTANT.properties"
                } );
            ...
            //( XProperties props, String chartFilename, Julian begin, Julian end, LogFile logger )
            ChartTree chartTree = new ChartTree(props, chart, logger );
            ChartElement elements[] = chart.getElementList();
            //new DisplayTree( "Chart of Accounts", tree, new Point(200,200), new Dimension(400, 1000) );
        ...
        extends DefaultMutableTreeNode
    {
        ...
        {
            try { return formatNode( element, NODE_DESCR_SPEC ); }
