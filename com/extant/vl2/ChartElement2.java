...
public class ChartElement2
{
    ...
    throws VLException
    {
        if ( name.equals( "account" ) ) return accountIndex;
        else throw new VLException
            ( VLException.INCOMPATIBLE_ACCOUNT, "getAccountIndex from " + name + " " + getAttribute( "no" ) );
    }

    public void setTotal( long total )
    throws VLException
    {
        if ( name.equals( "total" ) ) this.total = total;
        else throw new VLException
            (VLException.INCOMPATIBLE_ACCOUNT, "setTotal in " + name + " " + getAttribute( "no" ) );
    }

    public long getTotal()
    throws VLException
    {   if ( name.equals( "total" ) ) return total;
        else throw new VLException
            (VLException.INCOMPATIBLE_ACCOUNT, "getTotal from " + name + " " + getAttribute( "no" ) );
    }
    ...
    {
        String answer = name + ";" + Strings.format( level, "00" ) + ";"
            + Strings.format( accountIndex, "00" ) + ";" ;
