...
{
    public AccountFinder( Chart chart, LogFile logger, javax.swing.JComboBox comboBox )
    {
        this.chart = chart;
    ...
    public Account[] find( String clue )
    {
        ...
        // Order the output list either by account number or description, depending on first char of clue
        int ip[];
        if ( Strings.isAlpha( clue.charAt( 0 ) ) )
        {
            String descrs[] = new String[answerV.size()];
            for (int i=0; i<descrs.length; ++i) descrs[i] = ((Account)answerV.elementAt( i )).getTitle();
            ip = Sorts.sort( descrs );
        }
        else ip = Sorts.sort( answerV ); // uses the toString method in Account
        Account answer[] = new Account[answerV.size()];
        for (int i=0; i<answerV.size(); ++i) answer[i] = (Account)answerV.elementAt( ip[i] );
        return answer;
    }
    ...
    public void processKeyEvent(java.awt.event.KeyEvent evt)
    {
        ...
        else buffer.append( evt.getKeyChar() );
//Console.println( "AccountFinder.processKeyEvent( " + evt.getKeyChar() + " )  buffer=" + buffer.toString() );

        if ( buffer.length() == 0 ) initializeComboBox();
        else
        {
            comboBox.removeAllItems();
     ...
     * AccountFinder
     *    d = accounting directory
     *    f = chart file name
     *    x = what to look for
     */
    /***** FOR TESTING *****
    // This will test the matching logic, but not the GUI helper functions
    // (see AccountFinderTest)
    public static void main(String[] args)
    {
        try
        {
            Clip clip = new Clip( args, new String[] { "d=E:REMOTES\\QUITO\\GL08\\",
                                                       "f=CHART.XML",
                                                       "x=HANGAR" } );
            if ( clip.getParamCount() != 3 )
            {
                Console.println( "Use: AccountFinder <accounting-dir> <chart filename> <clue>" );
                return;
            }
            Chart chart = new Chart();
            chart.init( clip.getParam( "d" ) + clip.getParam( "f" ) );
            LogFile logger = new LogFile();
            logger.setLogLevel( LogFile.DEBUG_LOG_LEVEL );
            Account answer[] = new AccountFinder( chart, logger, null ).find( clip.getParam( "x" ) );
            if ( answer == null ) { Console.println( "No match found." ); return; }
            for ( int i=0; i<answer.length; ++i)
                Console.println( "[" + i + "] " + answer[i] );
        }
