 ...
 * @author  jms
 */
public class ListMan4
    extends javax.swing.JFrame
{
    public ListMan4(XProperties props, String fileName)
    {
        this(props, fileName, false);
    }

    public ListMan4(XProperties props, String fileName, boolean selectEntry)
    {
        try
        {
            logger = VL2.logger;
            ...
            String fileType = "";
            if (fileName.contains("VENDOR")) fileType = "Vendor";
            else if (fileName.contains("CUSTOMER")) fileType = "Customer:";
            else if (fileName.contains("CONTACTS")) fileType = "Contacts";
            this.setTitle(VL2.props.getString("EntityName") + " " + fileType + " List");
    ...
    private void btnOKActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnOKActionPerformed
        ...
        if ( dataHasChanged )
        {
            MsgBox msgBox = new MsgBox( this, "Confirm Close",
                ...
                MsgBox.YES_NO );
            if ( msgBox.getCommand().equalsIgnoreCase("yes" ) ) this.dispose();
        }
        else
        {
            if (selectEntry)
    ...
    private void comboNamesItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_comboNamesItemStateChanged
        //Console.println( "ItemStateChanged: " + (String)evt.getItem() + "  " + evt.getStateChange() );
    ...
    private void comboNamesKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_comboNamesKeyTyped
        evt.consume();
        char c = evt.getKeyChar();
        logger.logDebug( "typed: " + c );
        if ( c == '\b' )
        {
            if ( comboBuffer.length() > 0 ) comboBuffer = comboBuffer.substring( 0, comboBuffer.length() - 1 );
            else return;
        }
        else if ( c == evt.VK_ESCAPE ) comboBuffer = "";
        else comboBuffer += c;
    ...
    throws IOException
    {
        ...
        while ( !file.EOF() )
        {
            ...
            {
                if ( i >= linesIn.length ) lines[j++] = "";
                else if ( !Strings.isValidInt( linesIn[i].trim() ) ) lines[j++] = linesIn[i].trim();
            }
...
//        this.setVisible(true);
    ...
    Vector <String[]> organize( Vector <String[]> items )
    {
        ...
        for (int i=0; i<items.size(); ++i) names.addElement( ((String[])items.elementAt(i))[0] );
        ...
        try
        {
            ...
            for (int i=0; i<entries.size(); ++i)
            {
                ...
                for (int j=0; j<entry.length; ++j)
                {
                    if ( j > 0 ) image += delim;
                    image += entry[j];
                    if ( entry[j].length() == 0 ) image += " "; // probably no longer required
                }
...
//                    "File Uploading", "Please wait while the file is uploaded ...", MsgBox.NO_BUTTONS );
...
//        }
    ...
    public String[] getPayee() { return getCurrentEntry(); }
    public String[] getCustomer() { return getCurrentEntry(); }
    public String[] getContact() { return getCurrentEntry(); }
    public String[] getVendor() { return getCurrentEntry();
    }
    ...
    {   // Builds an entry from the dialog lines, or null if they are all blank
        ...
        String x = "";
        for (int i=0; i<vendor.length; ++i) x += vendor[i];
        if ( x.length() == 0 ) return null; // nothing was entered (there is no cancel button)
        return vendor;
    }
     ...
     public static void main(String args[])
    {
        try
        {
            final Clip clip = new Clip(args, new String[]
            { "f=E:\\REMOTES\\QUITO\\GL08\\VENDOR.LST"
            , "d=|"
            } );
            ...
            java.awt.EventQueue.invokeLater(new Runnable()
            {
                public void run()
                {
                    ListMan4 listMan = new ListMan4(clip.getParam("f"));
                    listMan.setDelim(clip.getParam("d"));
                    listMan.setVisible(true);
                }
            });
        }
