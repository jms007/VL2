    ...
    implements Enumeration
{
    ...
    /*
    init method creates a Chart with all accounts as specified in the 
    specified chart.xml input file.  No 'amount' information is included.
    */
    public void init( String chartFilename, LogFile logger )
    throws IOException, VLException
    {
        ...
        try
        {
            ...
            for (int i=0; i<nAccounts; ++i) acctsByChartP[i] = i;
            allElementsP = new int[chartElements.size()];
            for (int i=0; i<chartElements.size(); ++i) allElementsP[i] = i;
            ...
            for (int i=0; i<acctsByDescrP.length; ++i)
            {
                ...
                {
                    reportError( "[" + account.getAccountNo() + "] '" +
                        title + "'   Account Title cannot start with a number." );
                    titleError = true;
                }
            }
            if ( titleError ) throw new VLException (VLException.CHART_FORMAT_ERROR, "" );
            if ( plAccount == null )
            {
        ...
        throws SAXException
    {
        ...
        {
            logger.logDebug( "Creating Account: " + attrs.getValue( "no" ) +
            ...
            Account account = new Account
                ( attrs.getValue( "no" )
                , Strings.format( level, "00" )
                , attrs.getValue( "type" )  // this should be labeled 'class'
                , attrs.getValue( "title" )
                );
            ...
            {
                if ( plAccount == null ) plAccount = account;
                else reportError( "Multiple P/L Accounts are defined." );
            }
        }
                          ...
                          )
    {
        if ( qName.trim().equals( "chart" ) )
        {
            ...
            if (chartElements == null) logger.logFatal("Chart:237: chartElements is null!");
            if (elementList == null) logger.logFatal("Chart:237: elementList is null!");
    ...
    public Account findAcctByNo( String normalizedAcctNo )
    {
        ...
        if ( normalizedAcctNo == null ) return null;
...
//Console.println( "[Chart.match] partial='" + partial + "' lookForNumber=" + lookForNumber );
        int i;
        for (i=0; i<nAccounts; ++i)
        {
            if ( lookForNumber ) trial =
                ((Account)accounts.elementAt( acctsByNumberP[i] ) ).getAccountNo();
            else trial = 
                ((Account)accounts.elementAt( acctsByDescrP[i] ) ).getTitle();
            if ( trial.toLowerCase().startsWith( partial.toLowerCase() ) ) break;
        }
//Console.println( "   i=" + i + "  fail=" + (i>=nAccounts) );
        if ( i < nAccounts )
        {
            if ( lookForNumber ) p = acctsByNumberP[i];
            else                 p = acctsByDescrP[i];
//Console.println( "   returning " + ((Account)accounts.elementAt( p )).toString() );
...
//        }
    ...
    private Vector <String> buildAcctsByNumber()
    {
        ...
        for (int i=0; i<accounts.size(); ++i)
        {
            ...
            if ( st.hasMoreElements() ) numbers.addElement( (String)st.nextElement() );
        }
        return numbers;
    }
    ...
    {   
        ...
        for (int i=0; i<elementList.length; ++i) elementList[i] = (ChartElement)chartElements.elementAt( i );
        return elementList;
    }
    ...
    {   boolean old = this.showacct; this.showacct = showacct; return old; }
    ...
    private void reportDupAccounts( Vector <Account> accounts, int p[] )
    {
        ...
        {
            if ( ((Account)accounts.elementAt( i )).getAccountNo().equals
                ( ((Account)accounts.elementAt( i+1 )).getAccountNo() ))
            {
    ...
    public static void main(String[] args)
    {
        ...
        try
        {
            Clip clip = new Clip( args, new String[]
                { "in=G:\\ACCOUNTING\\JMS\\ARCHIVES\\GL06\\CHART.XML"
                });
            ...
            com.extant.utilities.Console.println( "maxAcctNoLength=" + chart.getMaxAcctNoLength() +
                "  '" + chart.getMaxAcctNo() + "'" );
        ...
        {   if ( chart != null ) com.extant.utilities.Console.println( chart.getErrorReport() );
//            x.printStackTrace();
