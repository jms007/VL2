...
//                         // V - Version of chart
        ...
        )
    {
        ...
        if (this.accountNo == null) logger.logFatal("Account constructor accountNo is null");
...
//    }
    ...
    /* a normalized account number is of the form
...
//
    ...
    // The next 2 methods (addToBal and updateBalances) are not currently (5-10-03) used
    // and have not been tested.
    public void addToBal( Julian begin, Julian end, long value, Julian trDate )
    {
        if ( trDate.isLaterThan( end ) ) return;
        if ( trDate.isEarlierThan( begin ) ) addToBeginBal( value );
        else addToDeltaBal( value );
    }
    ...
    public void updateBalances( Julian begin, Julian end )
    {
        ...
        for (int i=0; i<glEntries.size(); ++i)
        {
            ...
            addToBal( begin, end, glEntry.getSignedAmount(),
                glEntry.getFixedJulianDate() );
        }
    }
