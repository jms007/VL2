...
public class TranReport
    extends JDialog
{
    public TranReport( int reportType, Chart chart, XProperties props, LogFile logger )
    ...
    throws IOException, VLException
    {
        String image;
        GLEntry glEntry;
        workDir = props.getString("WorkDir");
        if (reportType == DETAIL) outFilename = workDir+"DetailTranReport.txt";
        else if (reportType == SUMMARY) outFilename = workDir+"SummaryTranReport.txt";
        ...
        dateFormat = props.getString( "ReportDateFormat", "mm-dd-yyyy" );
        // Default is to include all transactions
        begin = new Julian(props.getString("EarliestDate"));
        end =   new Julian(props.getString("LatestDate"));
    ...
    throws IOException, VLException
    {
        ...
        if ( begin.isLaterThan( end ) )
            throw new VLException( VLException.INCONSISTENT_DATES,
                "Begin Date (" + begin.toString( dateFormat ) + ") is later than End Date (" +
                    end.toString( dateFormat ) + ")" );
        outfile = new UsefulFile( outFilename, "w" );
        printTitle(outfile, reportType, begin.toString(dateFormat), end.toString(dateFormat), props);
        // Compute balances and add GL transactions to the accounts
        logger.logDebug("calling extractBalances begin="+begin+"   end="+end);
        VLUtil.extractBalances( props.getProperty("GLFile"), chart, begin, end, logger );
...
//    "   delta=" + chart.getPLAccount().getDeltaBal() + "   end=" + chart.getPLAccount().getEndBal() );
        //logger.setLogLevel(LogFile.DEBUG_LOG_LEVEL);
        ...
        while ( accounts.hasMoreElements() )
        {
            ...
            report( acctTotalLine( reportType, account, account.getBeginBal(),
                account.getDeltaBal(), account.getEndBal() ) );
        }
    ...
    private void printTitle( UsefulFile outfile, int reportType, String sDate, String eDate, XProperties props )
    {
        int reportWidth;
        if ( reportType == DETAIL ) reportWidth = 81;
        else reportWidth = 21 + 1 + chart.getMaxDescrLength() + 1 + 3*15;
        outfile.println( Strings.center( props.getString( "EntityLongName" ), reportWidth ) );
        ...
        outfile.println( "" );
        if (reportType == SUMMARY) outfile.println("                                                            " +
    ...
    private String detailLine( int reportType, GLEntry glEntry )
    {
        ...
        if ( reportType == DETAIL )
            image = "      " + 
                glEntry.getFixedJulianDate().toString( dateFormat ) + " " +
                glEntry.getField( "JRef" ) + " " +
    ...
    private String acctTotalLine( int reportType, Account account, long beginBal, long transTotal, long acctTotal )
    {
        ...
        if ( reportType == DETAIL )
            ...
            image = Strings.leftJustify( account.getAccountNo(), 22 ) +
                    Strings.leftJustify( account.getTitle(), 30 ) + " " +
                    Strings.rightJustify( Strings.formatPennies( beginBal,   dollarFormat ), 15 ) +
                    Strings.rightJustify( Strings.formatPennies( transTotal, dollarFormat ), 15 ) +
                    Strings.rightJustify( Strings.formatPennies( acctTotal,  dollarFormat ), 15 );
//Console.println( "[TranReport.acctTotalLine] image.length()=" + image.length() );
        return image;
    }

    private String reportTotalsLine( int reportType, long beginTotal, long transTotal, long endTotal )
    {
        // NOTE THE REPORT TOTAL FOR ALL ACCOUNTS WILL INCLUDE THE NET INCOME AMOUNT FOR THE PERIOD
        String image;
        if ( reportType == DETAIL )
    ...
    /***** For Testing: 
    // Use: TranReport
    //   ini=<VL Initialization File>
    //   dir=<Work Directory>
    //   o=<output file name>
    //   t=DETAIL | SUMMARY
    //   b=<begin date>
    //   e=<end date>
    //   [-s] to produce output in CSV format //!! not implemented
    //
    public static void main(String[] args)
    {
        try
        {
            Clip clip = new Clip( args, new String[]
                { "props=E:\\ACCOUNTING\\EXTANT\\EXTANT.properties"
                , "dir=E:\\ACCOUNTING\\EXTANT\\GL17\\"
                //, "o=GLSUM.TXT", "t=SUMMARY"
                , "o=GLDTL.TXT", "t=DETAIL"
                //, "b=1-1-17", "e=12-31-17"
                }
            );
            LogFile logger = new LogFile();
            logger.setLogLevel(LogFile.DEBUG_LOG_LEVEL);
            XProperties props = new XProperties( clip.getParam( "props" ));
            String workDir = clip.getParam( "dir" );
            props.setProperty( "WorkDirectory", workDir );
            String chartFileName = props.getString( "ChartFile" );
            Chart chart = new Chart();
            chart.init( chartFileName );
            logger.logDebug(
                "new GLCheck" +
                "(" +
                props.getString("GL0010") + " " +
                props.getString("CHART") + " " +
                "token" + " " +
                "props" + " " +
                "logger"
                    + ")"
            ...
            glCheck
                ( props.getString("GL0010")
                , props.getString("CHART")
                , "token"  // "fixed" or "token"
                , props
                , logger
                );
            TranReport tranReport = new TranReport();
            int reportType=0;
            if ( clip.getParam( "t" ).equalsIgnoreCase( "DETAIL" ) ) reportType = TranReport.DETAIL;
            else if ( clip.getParam( "t" ).equalsIgnoreCase( "SUMMARY" ) ) reportType = TranReport.SUMMARY;
            else
            {
                Console.println( "Invalid Report Type: " + clip.getParam( "t" ) );
                System.exit( 1 );
            }
            String outFilename = workDir + "TranReport.txt";
            tranReport.makeReport
                ( reportType
                , outFilename
                , chart
                , new Julian( clip.getParam( "b" ) )
                , new Julian( clip.getParam( "e" ) )
                , props
                , null
                );
            Console.println( "G/L Transaction Report is in " + outFilename);
        }
