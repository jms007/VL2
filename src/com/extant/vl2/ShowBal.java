    ...
    implements TreeSelectionListener
{
    ...
    {//GEN-HEADEREND:event_btnComputeActionPerformed
        try { compute(); }
        catch (VLException vlx) { statusBar.setText( vlx.getMessage() ); }
    ...
    throws VLException
    {
        ...
        try
        {
            ...
            {
                if ( cutoffDate == null )
                    lblAnswer1.setText( "Balance as of " + lastDate.toString( "mm-dd-yyyy" ) + ":" );
                else
        ...
        catch (IOException iox) { statusBar.setText( iox.getMessage() ); }
    }

    void manageButtons( java.awt.event.ActionEvent evt )
    {
        JRadioButton button = rbAllTrans;
        if ( evt == null ) button = rbAllTrans;
        else if ( evt.getActionCommand().startsWith( "Include" ) ) button = rbAllTrans;
        else if ( evt.getActionCommand().startsWith( "Specify" ) ) button = rbCutoffDate;
        ...
        if ( rbCutoffDate.isSelected() ) txtEffDate.requestFocus();
    }
