...
public abstract class AbstractStatement2
{
	public AbstractStatement2()
	{
	}

	public void setup(VL2Config vl2Config, Chart chart, boolean showAmounts, int reportLevel, String outfileName // specify
																													// ""
																													// if
																													// output
																													// is
			...
			, LogFile logger) throws VLException
	{
		try
		{
			...
			begin = vl2Config.getEarliestDate();
			end = vl2Config.getLatestDate();
			// if (reportLevel <= 0)
			// reportLevel = chart.getMaxLevel();
			if (showAmounts)
			{
				logger.logDebug("   calling VLUtil.computeAccountBalances(" + glFilename + ",<chart>," + begin + ","
						+ end + ")");
	...
	public String makeStatement() throws VLException
	{
		...
		Enumeration<ChartElement2> e = chart.chartElements.elements();
		e.hasMoreElements();
		{
			ChartElement2 element = e.nextElement();
			// ++nElements;
			if (listElements)
	...
	void processChartElement(ChartElement2 element) throws VLException
	{
		try
		{
			...
			{
				element.setBegin(levelTotals[element.getLevel()]);
	...
	private void printNetWorth(ChartElement2 element, Account thisAccount, boolean showAmounts) throws VLException
	{
		if (!showAmounts)
			return;
		String label = "R/E at " + begin;
	...
	private Vector<ChartElement2> cPrintBuffer = new Vector<ChartElement2>(10, 10);

	public void cPrint(ChartElement2 element, Account account, boolean conditional) throws VLException
	{
		...
		if (conditional)
		{
			...
			{
				int cPrintLevel = ((ChartElement2) cPrintBuffer.elementAt(i)).getLevel();
				String cPrintImage = ((ChartElement2) cPrintBuffer.elementAt(i)).getAttribute("title");
		...
		} else
		{
			for (int i = 0; i < cPrintBuffer.size(); ++i)
			{
				...
				{
					printTextLine((ChartElement2) cPrintBuffer.elementAt(i));
				}
				cPrintBuffer.removeElementAt(i);
				--i;
			}
	...
	abstract void printTextLine(ChartElement2 element) throws VLException;

	abstract void printAmountLine(ChartElement2 element, Account account) throws VLException;
