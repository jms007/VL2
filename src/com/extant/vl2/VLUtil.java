...
//    }
    ...
    /* fixDate returns a valid Julian date for the intentional invalid dates used in GL files:
     *   for yy0100 fixDate returns 12/31/<yy-1>
     *   for yy1232 fixDate returns 1/1/<yy+1>
     *   for valid dates, fixDate just returns the valid Julian
     *
     * Actually, I don't think we need this at all ... Julian seems to do this without
     * coaching.
     */
    public static Julian fixDate( String sDate )
    {
        if ( Julian.isValid( sDate ) ) return new Julian( sDate );
        Julian thisDate;
    ...
    /* The glExtract methods return a vector of GLEntry objects which meet the specified
     * selection criteria, sorted by account, subAccount, date.
     * For each (cols,data) pair, the entry is selected if the gl record contains the
     * String data[i] starting in column cols[i].
     *
     * Except for
     *   glExtract( String glFileName, int cols[], String data[], Julian begin, Julian end )
     * these routines have not been tested and are not currently (11-29-02) used.
     */

//    public static Vector glExtract( String glFileName, String fieldName, String equals )
//    throws IOException
//    {
//        return glExtract( glFileName, fieldName, equals, null, null );
//    }
//
//    public static Vector glExtract( String glFileName, String fieldName, String equals, Julian begin, Julian end )
//    throws IOException
//    {
...
//    }
//
//    public static Vector glExtract( String glFileName, int cols[], String data[], Julian begin, Julian end )
...
//    }
    ...
    /* extractBalances computes the beginning balance (as of the begin date)
     * and delta amount (up to and including the end date)
     * in each account contained in chart.
     * These values are stored in the accounts contained in the Chart.
     *
     ...
     * follow this call with a call to OopStatement.ComputeTotals
     * which will set the totals in the appropriate ChartEntry's
     */
    public static int extractBalances( String glFileName, Chart chart,
        Julian begin, Julian end, LogFile logger )
    throws IOException, VLException
    {
        ...
        if (plAccount == null) logger.log("VLUtil:189 plAccount is null!");
        UsefulFile glFile = new UsefulFile( glFileName, "r" );
    ...
    //Console.println("currentAcctNo="+currentAcctNo + "    currentAccount=" + currentAccount);
                if ( currentAccount == null )
                {
                    throw new VLException(
                        VLException.ACCT_NOT_IN_CHART, "[VLUtil.extractBalances] " + currentAcctNo +
                        " (GL Line " + lineNo + ")" );
                }
            }
            ...
            if ( glEntry.getFixedJulianDate().isEarlierThan( begin ) )
            {
                ...
                if ( currentAccount.getType().equals( "I" )
                    || currentAccount.getType().equals( "E" ) )
        ...
        // (I think this is to make Statement show the correct ending balance in P/L Account)
        
        String s = Strings.formatPennies( -plAccount.getDeltaBal() );
        logger.logDebug("VLUtil.extractBalances: s="+s);
        
        GLEntry netIncome = new GLEntry( "CLOS", "  0", "E", "C", "100", plAccount.getAccountNo(),
            s, "Computed Net Income", end.toString( "yymmdd" ));
    ...
    public static void main( String[] args )
    {
        String option = null;
        if ( args.length < 1 )
        {   option = "extract"; }    // Default
        else option = args[0];
        
        if ( option.equals( "date" ) )
        {
            String date = "020100";
            Console.println( date + " --> " + VLUtil.fixDate( date ).toString( "mm-dd-yyyy" ) );
            date = "011232";
            Console.println( date + " --> " + VLUtil.fixDate( date ).toString( "mm-dd-yyyy" ) );
        }
//        else if ( option.equals( "extract" ) )
//        {
//            String glFilename = "C:\\Test\\GL0010.DAT";
//            try
//            {
//                Vector entries = VLUtil.glExtract( glFilename,
//                    new int[] {0}, new String[] {"CD08"},
//                    new Julian( "1-1-02" ), new Julian( "8-20-02" ) );
//                Console.println( Strings.plurals( "Entry", entries.size() ) + " extracted." );
//                if ( Console.prompt( "Wanna see?", "N" ).equalsIgnoreCase( "Y" ) )
//                    for (int i=0; i<entries.size(); ++i)
//                        Console.println( entries.elementAt( i ).toString() );
//            }
//            //catch (IOException iox) { Console.println( iox.getMessage() ); }
//            //catch (VLException vlx) { Console.println( vlx.getMessage() ); }
//            Console.println( "Done" );
//        }
        else
        {
            Console.println( args[0] + " is not a valid parameter." );
            System.exit( 1 );
        }
        System.exit( 0 );
    }
