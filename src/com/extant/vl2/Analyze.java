	...
	implements TreeSelectionListener
{
    ...
    private void initComponents() {
        ...
        comboAccounts.setFont(new java.awt.Font("Dialog", 0, 12)); // NOI18N
        comboAccounts.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyTyped(java.awt.event.KeyEvent evt) {
                comboAccountsKeyTyped(evt);
            }
        });
        ...
        jPanel2.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Select Period to Analyze", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Arial", 0, 10))); // NOI18N
        ...
        jPanel3.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Select Desired Order", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Arial", 0, 10))); // NOI18N
    ...
    {//GEN-HEADEREND:event_manageSortButtons
        ...
        if ( evt == null ) button = rbSortByDate; // Default
        else if ( evt.getActionCommand().startsWith( "Description" ) ) button = rbSortByDescr;
        else if ( evt.getActionCommand().startsWith( "Date" ) ) button = rbSortByDate;
        else if ( evt.getActionCommand().startsWith( "Amount" ) ) button = rbSortByAmount;
    ...
    {//GEN-HEADEREND:event_manageDateButtons
        JRadioButton button = null;
        if ( evt == null ) button = rbAllTrans; // Default
        else if ( evt.getActionCommand().startsWith( "Include" ) ) button = rbAllTrans;
        else if ( evt.getActionCommand().startsWith( "Specify" ) ) button = rbDates;
        ...
        if ( rbDates.isSelected() ) txtStartDate.requestFocus();
    ...
    {//GEN-HEADEREND:event_btnAnalyzeActionPerformed
        try
        {
            if ( comboAccounts.getSelectedIndex() < 0 ) 
                throw new VLException(VLException.NO_ACCOUNT_SELECTED);
            if ( rbDates.isSelected() )
                analyze( (Account)comboAccounts.getSelectedItem(),
                    new Julian( txtStartDate.getText() ),
                    new Julian( txtEndDate.getText() ));
            else
                analyze( (Account)comboAccounts.getSelectedItem(), null, null );
        }
...
//        {
//            if ( comboAcctBuffer.length() > 0 ) comboAcctBuffer.deleteCharAt( comboAcctBuffer.length()-1 );
//        }
        ...
        try
        {
            ...
            if ( rbSortByDate.isSelected() || rbSortByAmount.isSelected() )
            {
                for (int i=0; i<p.length; ++i)
                {
                    ...
                    results.append(
                        thisEntry.getJulianDate().toString( dateFormat ) + " " +
                        thisEntry.getField( "JREF" ) + " " +
                        thisEntry.getField( "DRCR" ) + " " +
                        thisEntry.getNormalizedAccountNo() + " " +
                        thisEntry.getField( "DESCR" ) + " " +
                        thisEntry.getField( "AMOUNT" ) + " " +
                        "\n" );
            ...
            else if ( rbSortByDescr.isSelected() )
            {
                ...
                for (int i=0; i<p.length; ++i)
                {
                    ...
                    if ( ckboxSubtotal.isSelected() )
                    {
                        ...
                        if ( !thisDescr.equals( lastDescr ) )
                        {
                            ...
                            {
                                results.append(
                                    Strings.leftJustify( " ", dateSpace ) + "SubTotal " +
                ...
                if ( ckboxSubtotal.isSelected() )
                {
                    results.append( Strings.leftJustify( " ", dateSpace ) + "SubTotal " +
                        Strings.leftJustify( lastDescr, descrWidth ) +
                        Strings.rightJustify( Strings.formatPennies( subtotal, "" ), 26 - dateSpace ) + "*\n \n" );
                    if ( !lastPayer.equals( "" ) ) results.append( Strings.leftJustify( " ", dateSpace ) + "Payer Total " +
                        Strings.leftJustify( lastPayer, 10 ) +
                        Strings.rightJustify( Strings.formatPennies( payerSubtotal, "" ), 42 - dateSpace ) + "*\n \n" );
                }
            }
...
//
    ...
    private javax.swing.JComboBox comboAccounts;
