 ...
 * GLEntry.java
 *
 * Builds one variable-length GLEntry from the given data.
...
public class GLEntry
{
    ...
    public int getIndex(String fieldName)
    {
        for (int i=0; i<nFields; ++i)
            if (fieldName.equalsIgnoreCase(fieldNames[i])) return i;
        return -1;
    }
...
//    }
    ...
    {
        if ( this.isCredit() ) return -Strings.parsePennies( getField("Amount") );
        else return Strings.parsePennies( getField("Amount") );
    }
    ...
    {
        if ( getField("DRCR").equals( "C" ) ) setField("DRCR", "D");
        else setField("DRCR", "C");
    ...
    {
        String record = "";
        for (int i=0; i<nFields; ++i) record += fields[i] + "|";
        return record;
    }
    ...
    {
        // Try regex
        // String javaPattern =  "[A-Z]{4}\\|0\\|E\\|[CD]\\|100\\|[\\d]{4}(/.*){0,1}\\|\\d*\\.\\d\\d\\|.*\\|\\d{6}";
    ...
    public static void main( String[] args )
    {
        ...
        {
            String fieldName;
            //String image = "TEMP  0ED100100       ENCIRQ       29512.39TEMP BAL FWD.................020101";
            String image = "TEMP|0|E|D|100|100|ENCIRQ|29512.39|TEMP BAL FWD|020101";
            GLEntry glEntry = new GLEntry( image );
            for (int i=0; i<fieldNames.length; ++i)
            {
                fieldName = GLEntry.fieldNames[i];
                Console.println( fieldName + "=" + glEntry.getField( fieldName ) );
            }
            Console.println( "pennies=" + glEntry.getLongAmount() );
            Console.println( "JDate=" + glEntry.getJulianDate().toString( "mm-dd-yyyy" ) );
            Console.println( "'" + glEntry.buildRecord() + "'" );
            Console.println( "valid=" + glEntry.validate() );
        }
...
/* Format of (old) fixed-length general ledger file records.
   The fields are in the same and in the same order EXCEPT 
    (a) subAccount has been removed (combined with account), and
    (b) GSN has been added to the format.
   The fields are separated in the new format by the '|' delimiter.
   These column numbers are obviously not relevant to the variable-length format.
0         1         2         3         4         5         6         7
012345678901234567890123456789012345678901234567890123456789012345678901234567
TEMP  0ED100100.......ENCIRQ....12345678.99TEMP BAL FWD.................020100

 *  Index    Col      Field
 *    0     0 -  3    Journal Reference
 *    1     4 -  6    GSN
 *    2     7 -  7    Status
 *    3     8 -  8    Debit/Credit
 *    4     9 - 11    Company Number
 *    5    12 - 21    Account
 *    6    22 - 31    SubAccount
 *    7    32 - 42    Amount
 *    8    43 - 71    Description
...
//    public final static String DATE = "date";
    ...
    public final static String[] fieldNames = new String[]
        {"JREF", "GSN", "STATUS", "DRCR", "CONUM",
         "ACCOUNT", "AMOUNT", "DESCR", "DATE"};
    public final static int nFields = fieldNames.length;
    //public final static int[] pStart = new int[] {0, 4, 7, 8,  9, 12, 22, 32, 43, 72};
    //public final static int[] pEnd =   new int[] {3, 6, 7, 8, 11, 21, 31, 42, 71, 77};
