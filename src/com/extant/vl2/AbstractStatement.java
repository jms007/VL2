 ...
 *
 * To implement, extend this class with the methods:
 *   initialize()
 *   processAnnotation()
 *   formatLine()
 *   close()
 *
 * Created on February 20, 2005, 12:37 PM
 */
...
//              this.outfileName = workDir + outfileName;
            ...
            if (showAmounts)
            {
                ...
                for (int i=0; i<levelTotals.length; ++i) levelTotals[i] = 0L;
            }
        ...
        catch (IOException iox) { throw new VLException( VLException.IOX, iox.getMessage() ); }
    }
    ...
    void processChartElement( ChartElement element )
    throws VLException
    {
        try
        {
            ...
            {   // Print ending balance in this account & add to level total
                ...
                if ( element.getAttribute( "type" ).equals( "R" ) ) printNetWorth( element, account, showAmounts );
            ...
            else if ( elementName.equals( "total" ) && showAmounts)
            {
                ...
                Account account = new Account( "", element.getLevelString(), 
    ...
    private void printNetWorth( ChartElement element, Account thisAccount, boolean showAmounts )
    throws VLException
    {
    ...
    public void cPrint( ChartElement element, Account account, boolean conditional )
    throws VLException
    {
        if ( conditional )
        {
