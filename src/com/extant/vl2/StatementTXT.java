...
extends AbstractStatement
{
    ...
    throws VLException
    {
        setup( props, chart, true, earliestDate, latestDate, reportLevel, outfileName, logger );
    }
    ...
    {   // Prints lines containing text only
        ...
        if ( format.contains( "newpage" ) ) image += "\f";
        if ( format.contains( "center" ) )
            image += Strings.center( element.getAttribute( "title" ), maxLineLength );
        else
        {
            if ( chart.getShowacct() ) image += Strings.leftJustify( " ", chart.maxAccountNo.length() + 1 );
        ...
        if ( format.contains( "period" ) )
            image += "\r\n" + Strings.center( begin.toString( dateFormat ) + " - " +
                end.toString( dateFormat ), maxLineLength );
        if ( format.contains( "skip" ) ) image += "\r\n";
        outfile.println( image );
    }

    @Override // implements method from vl2.AbstractStatement
    void printAmountLine( ChartElement element, Account account )
    throws VLException
    {
        outfile.println( formatAmountLine( element, account ) );
    }

    String formatAmountLine( ChartElement element, Account account )
    throws VLException
    {   // Formats lines with amounts
        String image = "";
        int formatLevel = account.getLevel();
        if ( account.getType().equalsIgnoreCase( "T" ) ) --formatLevel;
        if ( chart.getShowacct() )
        {
            if ( account.getType().equalsIgnoreCase( "A" ) )
                image += Strings.leftJustify( account.getAccountNo(),
                    chart.maxAccountNo.length() ) + " ";
            else
        ...
        if ( element != null )
            if ( element.getAttribute( "format" ).contains( "skip" ) ) image += "\r\n";
    ...
    public void calculateColumns( Chart chart, int reportLevel )
    throws VLException
    ...
    public static void main(String[] args)
    {
        try
        {
            ...
            String chartFilename = clip.getParam( "c" );
            if ( chartFilename.indexOf( File.separator ) < 0 )  chartFilename = workDir + chartFilename;
            ...
            if ( outfileName.indexOf( File.separator ) < 0 ) outfileName = workDir + outfileName;
            // For Debugging:
            ...
            statementTXT.makeStatement();
            System.out.println( "Output is in file " + outfileName );
        }
        catch (Exception x)
        {   x.printStackTrace(); }
