...
//        }
        if (fileType.equals("fixed")) checkFixedFile( glFilename );
        else if (fileType.equals("token")) checkTokenFile( glFilename );
        else logger.logFatal("GLCheck: Invalid File Type = " + fileType);
        return nErrors;
    }
    ...
    public void checkTokenFile( String glFilename )
    {
        ...
        { 
            ...
            while ( !f.EOF() )
            {
                image = f.readLine();
                ++lineNo;
                if (image.length() < 3) continue;  // Ignore blank lines
                ...
                if ( earliestDate == null ) earliestDate = glEntry.getFixedJulianDate();
                else if ( glEntry.getFixedJulianDate().isEarlierThan( earliestDate ) )
                    earliestDate = glEntry.getFixedJulianDate();
                if ( latestDate == null ) latestDate = glEntry.getFixedJulianDate();
    ...
    {   
        ...
        public void finish()
        {
            ...
            {
                report += lineNo + " entries, " +
                    nJournals + " Journals.\n" + 
                    "Covering dates " + earliestDate.toString( "mm-dd-yyyy" ) +
                    " to " + latestDate.toString( "mm-dd-yyyy" ) + "\n";
                props.setProperty("EarliestDate", earliestDate.toString("yymmdd"));
                props.setProperty("LatestDate", latestDate.toString("yymmdd"));
            }
            else
...
//                    nJournals + " Journals.\n";
            reportJournalBal();
            if ( bal != 0L )
                reportError( "General Ledger is out of balance by " +
                    Strings.formatPennies( bal, "," ) );
            if (nErrors > 0) report += Strings.plurals( "Error", nErrors ) + " found.\n";
            else logger.logDebug("GLCheck normal completion");
        }
    ...
    {
        GLCheck glCheckinstance = new GLCheck();
        glCheckinstance.glCheck
            ( "G:\\ACCOUNTING\\JMS\\ARCHIVES\\GL06\\GL0010.DAT"
            , "G:\\ACCOUNTING\\JMS\\ARCHIVES\\GL06\\CHART.XML"
            , "token"
            , new LogFile()
            );
        
        Console.print( glCheckinstance.getReport() );
    }
    /***** END OF TESTING *****/
}
