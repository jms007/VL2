...
public class StatementPDF extends AbstractStatement {
	...
	void addTextLine(ChartElement element) throws BadElementException
	...
	public void printAmountLine(ChartElement element, Account account) throws VLException
	{
		...
		try {
			...
			if (account.getType().equalsIgnoreCase("L") || account.getType().equalsIgnoreCase("I")
				...
				bal = -bal;
			for (int i = 0; i < reportLevel - accountLevel; ++i)
				cells.addElement(new Cell(""));
	...
	/**
	 * @param args
	 *            the command line arguments dir=<work directory> c=<chart file
	 *            name> gl=<GL file name> o=<output file name>
	 */
	/*****
	 * FOR TESTING ***** public static void main(String[] args) { try { Clip clip =
	 * new Clip( args, new String[] { "dir=E:\\ACCOUNTING\\EXTANT\\GL17\\" ,
	 * "c=CHART.XML" , "gl=GL0010.DAT" , "o=StmtOOP.pdf" } ); LogFile logger = new
	 * LogFile(); String workDir = clip.getParam( "dir" ); if ( !workDir.endsWith(
	 * File.separator ) ) workDir += File.separator; Chart chart = new Chart();
	 * chart.init( workDir + clip.getParam( "c" ) ); String glFilename =
	 * clip.getParam( "gl" );
	 * 
	 * // glCheck // ( String glFilename // , String chartFilename // , String
	 * fileType // "fixed" or "token" // , LogFile loggerParm // ) GLCheck glCheck =
	 * new GLCheck(); glCheck(clip.getParam("gl"), clip.getParam("c"), "token",
	 * logger); if ( glFilename.indexOf( File.separator ) < 0 ) glFilename = workDir
	 * + glFilename; glCheck.checkTokenFile( glFilename ); Julian begin =
	 * glCheck.getEarliestDate(); Julian end = glCheck.getLatestDate(); String
	 * outfileName = clip.getParam( "o" ); if ( outfileName.indexOf( File.separator
	 * ) < 0 ) outfileName = workDir + outfileName; // For Debugging:
	 * //logger.setLogLevel( logger.DEBUG_LOG_LEVEL );
	 * 
	 * StatementPDF statementPDF = new StatementPDF ( workDir , chart , glFilename ,
	 * begin , end , 0 // reportLevel , outfileName , logger );
	 * statementPDF.makeStatement(); com.extant.utilities.Console.println( "Output
	 * is in " + outfileName ); } catch (Exception x) { x.printStackTrace(); } } /
	 *****/
